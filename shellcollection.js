(function() {
  ...
  const ShellCollection = function(
    ...
    this.cntChangeCallback = cntChangeCallback;
    this.shellOnClick = onClickCallback || function() {};
    this.render = render;
    this.parse = parse;
    this.clear = clear;
    this.find = find;
  ...
  function add(type) {
    const elShell = new ElementShell(type, this);
  ...
  function ElementShell(type, collection) {
    const shell = {};
    ...
    function removeShell() {
      const index = this.list.indexOf(shell);
      parentElement.remove();
      this.list.splice(index, 1);
    }

    shell.remove = removeShell.bind(collection);
    ...
    shell.up = () => move.call(collection, true);
    shell.down = () => move.call(collection, false);

    function move(up) {
      const offset = up ? -1 : 1;
      const index = this.list.indexOf(shell);
      const siblingIndex = index + offset;
      const sibling = this.list[siblingIndex];

      if (sibling) {
        const temp = shell;
        this.list[index] = sibling;
        this.list[siblingIndex] = temp;
